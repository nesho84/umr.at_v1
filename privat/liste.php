...
tr { backgound: #cccccc; }
th { padding:2px; text-align: right; width: 46%; }
td { padding-left: 0px; backgound-color: #ccccff; }
.button { width: 140px; height: 51px; background: #3C74E6 url(../_images/img06.gif) repeat-x; font-size: 28px; font-weight: normal;
...
select { width: 225px; height: 47px; }
</style>
<table width="100%" border="0" align="" cellpadding="10" cellspacing="10" class="boxed"">
<tr>
<td height="40" style="text-align: left;"><h2><strong>Stundenliste - UMR GmbH <div id="hrblue"></div></strong></h2></td>
</tr>
</table>
...
<select name="name" id="name" class="large-font">
	...
	$qry2 = mysql_query("SELECT * FROM members where username='$username'");
	...
	{
	$qry=mysql_query("SELECT * FROM members where members_level='mitarbeiter'", $con);
	if(!$qry) { die("Query Failed: ". mysql_error()); }
	echo "<option value='".$name2."'>".$name2."</option>";
	while($row=mysql_fetch_array($qry)) { echo "<option value='".$row['name']."'>".$row['name']."</option>"; }
	}
	//If members_level = firma
	if ($members_level == 'firma') 
	{
	$qry3=mysql_query("SELECT * FROM members where members_level='vorarbeiter'", $con);
	$qry4=mysql_query("SELECT * FROM members where members_level='mitarbeiter'", $con);
	if(!$qry3 || !$qry4) { die("Query Failed: ". mysql_error()); }
	while($row3=mysql_fetch_array($qry3)) { echo "<option value='".$row3['name']."'>".$row3['name']."</option>"; }
	while($row4=mysql_fetch_array($qry4)) { echo "<option value='".$row4['name']."'>".$row4['name']."</option>"; }
	}
	?>
</select>
</h2><br />
...
<td><br /><h2 align="center"><input type="submit" class="button" value="Suchen" /></h2><br /></td>
</tr>
</table>
</form>
<br />
