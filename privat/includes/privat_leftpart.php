<style type="text/css">
ul { list-style: none; }
#sidebar li { margin-top: 8px; border: 1px solid #cccccc; border-radius: 10px; }
li a { height: 26px; padding-top: 8px; text-decoration: none; display: block; font-weight: bold; }
#sidebar li a:hover { background: #C6F2E5; border-radius: 10px; }
.contentleft li{ padding: 0px; }
</style>
	<?php
	//Counting numbers of news
	$count1=mysql_query("SELECT * FROM news order by date");	
	if(!$count1) { die("Query Failed: ". mysql_error()); }
	$num_rows = mysql_num_rows($count1);
	?>
<div id="content"> <!-- START CONTENT -->
<div id="sidebar">
<div id="updates" class="boxed">
<div class="title"><h2>Benutzer Navigation</h2></div>
...
<li><a <?php if ($active == 'News') { ?>class="active"<?php } ?> href="<?php echo SITE_URL ?>privat/News">News&nbsp;( <span style="color: red;"><?php echo "$num_rows \n"; ?></span> )</a></li>					
	...
	$qry = mysql_query("SELECT * FROM members where username='$username'");
...
<li><a <?php if ($active == 'Kennwort') { ?>class="active"<?php } ?> href="<?php echo SITE_URL ?>privat/Kennwort">Kennwort ändern</a></li>
...
Wenn Sie Fragen, Feedback für den Website haben oder einen Fehler auf unserer Website gefunden?<br /><br />
