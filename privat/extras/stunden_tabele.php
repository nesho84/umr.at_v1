...
tr { backgound: #cccccc; }
th { padding:2px; }
td { text-align:center; padding:2px; backgound-color: #ccccff; }
.button { width: 80px; background: #3C74E6 url(../_images/img06.gif) repeat-x; font-size: 12px; font-weight: normal;
color: #ffffff; border: 2px solid #c9e749; }
...
select { width: 304px; height: 36px; }
	...
	$qry = mysql_query("SELECT * FROM stundenliste where name='$name' AND monat='$monat' ORDER BY date,firma LIMIT 0, 61 ", $con);
	if(!$qry) { die("Query Failed: ". mysql_error()); }
	?>
<h4><img src="<?php echo IMAGE_DIR;?>/logo/logosmall-nn-gif.gif" border="0"></h4>
<h6 style="text-align: right;"><a href="javascript:window.print()"><img src="<?php echo IMAGE_DIR;?>/print.jpg" border="1"></a></h6>
<h1 style="margin-top: -24px">Stundenliste - UMR GmbH</h1><div id="hrblue"></div><br />
...
<th>Action</th>
	...
	?>
<tr bgcolor="#FFFFFF">
<td width="14%"><?php $old_date = $row['date']; echo $old_date; ?></td>
...
<td bgcolor="#ABAFB7" style="color:#ffffff;">
	...
	$result = mysql_query($query) or die(mysql_error());
	while($row = mysql_fetch_array($result)){
	echo "Ü.Std. <b>(" . $row['SUM(uber_stunden)'] . ")</b>";
	echo "<br />";
	}
	?>
</td>
