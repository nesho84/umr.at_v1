...
tr { backgound: #cccccc; } 
th { padding:2px; text-align: right; width: 46%; border: 2px solid #fff; }
...
input { font-size: 14px; width: 315px; height: 30px; }
...
<tr><th>Username:</th><td><input type="text" class="large-font" value="<?php echo $row['username'];?>" disabled /></td></tr>
<tr><th>Name:</th><td><input type="text" class="large-font" value="<?php echo $row['name'];?>" disabled /></td></tr>
<tr><th>E-mail:</th><td><input name="email" class="large-font" id="email" value="<?php echo $row['email'];?>" /></td></tr>
<tr><th>Telefon:</th><td><input name="tel" class="large-font" id="tel" value="<?php echo $row['tel'];?>" /></td></tr>
<tr><th>Addresse:</th><td><input name="address" class="large-font" id="address" value="<?php echo $row['address'];?>" /></td></tr>
<tr>
<td>&nbsp;</td><td align="left"><input name="edit" type="submit" class="button" id="update" value="&nbsp;Speichern&nbsp;"></td>
</tr>
	<?php
	}
	?>
	...
	$qry=mysql_query("UPDATE members SET email='$email', tel='$tel', address='$address' WHERE userID='$userID'", $con);
	if(!$qry)
	{
	...
	{
	echo "<br /><br />";
	echo "<p align=center style=color:green; font-size:small; class=box>Profile daten erfolgreich geändert...</p>";
	?>
