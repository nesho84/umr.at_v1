...
if (isset($_GET['userID'])) {
	$userID = $_GET['userID'];
	$qry = mysqli_query("SELECT * FROM members WHERE userID=$userID AND members_level='mitarbeiter'", $con);
	if (!$qry) {
...
?>
	<form method="post">
		<table width="100%" border="0" align="" cellpadding="1" cellspacing="1" class="boxed">
			<tr>
				<th height="40" align="left" style="text-align: left; border: 0px;">
					<h2><strong>Mitarbeiter - Bearbeiten<div id="hrblue"></div></strong></h2>
				</th>
			</tr>
		</table>
		<table width="100%" border="0" align="" cellpadding="10" cellspacing="10">
			...
			?>
				...
				<tr>
					<td></td>
					<td align="left"><span style="font-size:11px;">Eingabefelder mit einem ( * ) sind Pflichtfelder.</span></td>
				</tr>
				<tr>
...
if (isset($_POST['edit'])) {
	...
	$qry = mysqli_query("UPDATE members SET members_level='$members_level', email='$email', tel='$tel', address='$address' WHERE userID='$userID'", $con);
	if (!$qry) {
		die("Query Failed: " . mysqli_error($con));
	} else {
		echo "<br /><br />";
		echo "<p align=center style=color:green; font-size:small; class=box>Mitarbeiter daten erfolgreich ge�ndert...</p>";
?>
